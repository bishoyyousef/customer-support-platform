...
export const TicketList: React.FC<TicketListProps> = ({
  ...
  const [suggestions, setSuggestions] = useState<{ type: string; text: string; subtext?: string; ticketId?: string }[]>([]);
  ...
  // Combine items for keyboard navigation: suggestions if query >= 1, else search history
        ...
        <div style={{ ...styles.searchContainer, position: 'relative' }}>
          ...
          {showHistoryDropdown && (isSuggestionMode || searchHistory.length > 0) && (
            ...
            >
              {isSuggestionMode ? (
                <>
                  <div style={{ padding: '0.25rem 0.75rem 0.5rem', fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-text-muted)' }}>
                    SUGGESTIONS
                  </div>
